...
def load_data(file_name):
    try: 
        ...
        return df.where(pd.notnull(df), None).to_dict('records') # Xử lý lỗi giá trị trống (NaN)
    except: return []
    ...
    with st.form("reg_form"):
        ...
        if st.form_submit_button("Xác nhận đăng ký"):
            if u_id and pwd and name:
                ...
                data.append({"username": u_id, "password": pwd, "name": name, "class": lop, "role": "student"})
...
def main_app():
    ...
    if user.get('role') == "student":
        ...
        # (Các tab t1, t2, t3, t4 giữ nguyên như bản trước)
        with t1:
        ...
        with t2:
            ...
            if st.button("Xác nhận Hủy"):
                ...
                if ("Bữa Trưa" in buoi and gio >= 9) or ("Bữa Chiều" in buoi and gio >= 15): st.error("Quá giờ!")
                else:
                    save_all_data("nhat-ky.csv", load_data("nhat-ky.csv") + [{"Loại":"Báo ăn","Lớp":user['class'],"Tên":user['name'],"Nội dung":f"HỦY: {thu} {buoi}","Thời gian":datetime.now().strftime("%H:%M"),"Trạng thái":"Đã gửi","Ảnh":""}])
                    st.success("❌ Đã báo hủy!")
        with t3:
            ly_do = st.text_area("Lý do nghỉ:")
            a_ng = st.camera_input("Ảnh minh chứng")
            if st.button("Gửi đơn"):
                save_all_data("nhat-ky.csv", load_data("nhat-ky.csv") + [{"Loại":"Xin nghỉ","Lớp":user['class'],"Tên":user['name'],"Nội dung":ly_do,"Thời gian":datetime.now().strftime("%H:%M %d/%m"),"Trạng thái":"⏳ Chờ duyệt","Ảnh":image_to_base64(a_ng)}])
                st.success("✅ Đã gửi!")
        with t4:
            yk = st.text_area("Ý kiến:")
            if st.button("Gửi phản ánh"):
                save_all_data("nhat-ky.csv", load_data("nhat-ky.csv") + [{"Loại":"Phản ánh","Lớp":user['class'],"Tên":user['name'],"Nội dung":yk,"Thời gian":datetime.now().strftime("%H:%M"),"Trạng thái":"Đã gửi","Ảnh":""}])
                st.success("📩 Đã nhận!")
            ...
            if ds_sk:
                for idx, sk in enumerate(reversed(ds_sk)):
                    ...
                    with st.container(border=True):
                        ...
                        likes = int(sk.get('Likes') if sk.get('Likes') else 0)
                        if col_l.button(f"👍 Like ({likes})", key=f"lk_{r_idx}"):
                            ds_sk[r_idx]['Likes'] = likes + 1
                            save_all_data("su-kien.csv", ds_sk); st.rerun()
                        
                        # Xử lý Bình luận an toàn
                        raw_comments = sk.get('Comments')
                        try:
                            comments = json.loads(raw_comments.replace("'", '"')) if raw_comments and str(raw_comments) != 'nan' else []
                        except:
                            comments = []
                        ...
                        with st.form(f"f_c_{r_idx}", clear_on_submit=True):
                            new_com = st.text_input("Bình luận...", key=f"ic_{r_idx}")
                            if st.form_submit_button("Gửi ✈️"):
                                if new_com:
                                    comments.append({"user": user['name'], "text": new_com})
                                    ds_sk[r_idx]['Comments'] = json.dumps(comments, ensure_ascii=False)
                                    save_all_data("su-kien.csv", ds_sk); st.rerun()
        ...
        # (Phần Duyệt nghỉ, Điểm danh, Phản ánh giữ nguyên nút Duyệt như bản trước)
            ...
            for i, item in enumerate(nhat_ky):
                if item['Loại'] == loai:
                    with st.expander(f"✉️ {item['Tên']} - {item['Trạng thái']}"):
                        ...
                        if item.get('Ảnh'): st.image(base64.b64decode(item['Ảnh']), width=300)
                        c1, c2, c3 = st.columns(3)
                        with c1:
                            if st.button("Duyệt ✅", key=f"d_{loai}_{i}"):
                                nhat_ky[i]['Trạng thái'] = msg; save_all_data("nhat-ky.csv", nhat_ky); st.rerun()
                        with c2:
        ...
        with t_ng: render_admin("Xin nghỉ", "✅ BGH đã duyệt nghỉ!")
        ...
        with t_sk:
            ...
            with st.form("new_post"):
                ...
                if st.form_submit_button("ĐĂNG"):
                    save_all_data("su-kien.csv", load_data("su-kien.csv") + [{"Tiêu đề":tt,"Nội dung":nd,"Ảnh":image_to_base64(im),"Thời gian":datetime.now().strftime("%d/%m"),"Likes":0,"Comments":"[]"}])
                    st.success("Đã đăng!"); st.rerun()
            # Xóa bài sự kiện
            st.divider()
            ds_sk_adm = load_data("su-kien.csv")
            for i, sk in enumerate(ds_sk_adm):
                col_n, col_x = st.columns([0.8, 0.2])
                col_n.write(f"📌 {sk['Tiêu đề']}")
                if col_x.button("Xóa 🗑️", key=f"del_sk_{i}"):
                    ds_sk_adm.pop(i); save_all_data("su-kien.csv", ds_sk_adm); st.rerun()
